...
1. In the left pane, click on the *Data* icon > *Data Sources* to find the Data Source added earlier.
2. Copy the Python code snippet into the first line of your notebook and run the cell.
...
4. Now, cell by cell, copy the code snippets below and run the cells to visualize and prepare the data!

You can click on the '+' icon to add a blank cell after the current cell:
...
5. Finally write your data to a Domino Dataset by running the following:
...
6. Run all cells. Your notebook should be populated like the display below:
...
1. Click on the *File Changes* tab in the top left corner of your screen:
...
2. Click on your notebook to view it.
3. Click *Link to Task* in the top left.
4. Choose the *Explore Data* task we created in Lab 1.3.
...
5. Navigate back to the *Tasks* page, and select *Explore Data*.
You'll see our linked notebook.
6. Click on the circled checkmark to mark this complete. 
...
2. Type or paste the following command below in the *File Name or Command* section of the *Start a Job* pop up window.
3. Click *Start* to run the job.
...
We've now trained 3 models and it is time to select which model we'd like to deploy. Domino experiment management leverages MLflow Tracking to enable easy logging of experiment parameters, metrics, and artifacts. 

MLflow runs as a service in your Domino cluster, fully integrated within your workspace and jobs, and honoring role-based access control. Existing MLflow experiments work right out of the box with no code changes required!

The jobs that we just ran had MLFlow tracking in them to log the R^2 value and Mean Squared Error (MSE). To view the experiments:

1. Click on the *Experiments* tab in your project.
Here you have one set of experiments that all the jobs were logged against.
2. Click on the experiment name to see more details.
...
1. Click on the three dots and choose *Edit*.
...
#### Step One: Create model endpoint
...
1. Navigate to *Deployments > Endpoints* in your project.
2. Click *Create Domino endpoint*.
3. Name your model *wine-model-yourname*.
4. For the description, add the following: 
...
#### Step Two: Confirm endpoint details

1. In the field *The file containing the code to invoke (must be a Python or R file)* enter the following:

```
scripts/predict.py
```
    
2. In the field *The function to invoke* enter the following:
    
```
predict
```

3. Confirm that the *Choose an Environment* has the *WineQualityProject* selected.

4. Check the box *Log HTTP requests and responses to model instance logs*.
...
5. Switch back to your new file tab and paste the new lines in line 64 in your file.
...
#### Step Three: Publish and test your app

Now that you have your `app.sh` and `shiny_app.R` files created. 

1. Navigate to the *Deployments > App* tab in your project.
2. Enter a title for your app - *YourName Wine Quality Prediction*.
3. Click *Publish*.
                   
You'll now see the below screen. Once your app is active (generally within ~1-3 minutes), you can click the View App button. 
...
Once you're in the app you can try out sending different scoring requests to your model using the form on the right side of your page. 

4. Click *Predict* to send a scoring request .
5. View the results in the visualization on the left side.
...
3. Navigate back to the *Settings* tab.
4. Click *Copy App Link*.
5. Paste the copied link into a new private/incognito window.
Note that you're able to view the app without being logged into Domino.

Tip: Domino provides free licenses for business users to log in and view apps.
